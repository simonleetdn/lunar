...
<?php
    ...
    foreach ($years as $year) {
      for ($month = 1; $month <= 12; $month++) {
        $month_padded = str_pad($month, 2, '0', STR_PAD_LEFT); // Ensures the month is two digits
        echo "<url>\n";
...
foreach ($years as $year) {
	foreach ($activities as $activity) {
        $encoded_activity = urlencode($activity);  // URL encode the activity to ensure it's safe for URLs
        echo "  <url>\n";
